...
app_lab = pd.read_csv("F:/kaggle_data/talking_data_mobile/app_labels.csv", dtype={'device_id': np.str})
...
##################
print("# Read Events")
events = pd.read_csv("F:/kaggle_data/talking_data_mobile/events.csv", dtype={'device_id': np.str}, usecols=['event_id','device_id','timestamp'])
events['hour'] = pd.to_datetime(events.timestamp).dt.hour
events.hour = events.hour.astype(np.str) + 'h'
events["app_lab"] = events["event_id"].map(app_ev)
events['app_lab'] = events.hour + ' ' + events.app_lab
...
train = pd.read_csv("F:/kaggle_data/talking_data_mobile/gender_age_train.csv",
                    dtype={'device_id': np.str})
...
X_train, X_val, y_train, y_val = train_test_split(train, Y, train_size=.90)
...
gbm = xgb.train(params, dtrain, 80, evals=watchlist, verbose_eval=True, early_stopping_rounds = 10)
...
result.to_csv('F:/test_Result_70.csv', index=True, index_label='device_id')
